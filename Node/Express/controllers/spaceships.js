...
const getAllSpaceships = ( req = request, res = response ) => {
    res.status(200).json({
        spaceshipList
    });
}

const getSpaceship = ( req = request, res = response ) => {
    const id = req.params.id;

    if(id > spaceshipList.length || id < 0){
        res.status(400).json({
            result : "There is no spaceship with that id"
        });
    } else {
        res.status(200).json({
            result : spaceshipList[id]
        });
    }
}

module.exports = { getAllSpaceships, getSpaceship};